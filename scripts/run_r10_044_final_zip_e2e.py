#!/usr/bin/env python3
"""R10 0.4.4 final plugin-ZIP isolated E2E harness (opt-in).

Installs the landed 0.4.4 plugin ZIP into a temporary Hermes home and proves:
...
4. protocol-field registered-secret fail-closed (0.4.4 narrow);
5. fake Provider sees zero synthetic secrets; blocked cases Provider=0;
6. source-tree fallback mutation is load-bearing: damaged install + healthy
...
Landed phase: ``ARTIFACTS_LANDED=True`` with ``STRICT=True`` after dual-build
``copy2`` into ``dist/`` and real plugin ZIP SHA backfill. Resolve must bind
the landed ZIP hash (never silent skip / fake green). Pending-build hard-fail
path remains for historical contract documentation only.
...
# Strict stage flag: True after dual-build copy2 land (was pending sentinel).
ARTIFACTS_LANDED = True
STRICT = True
# Backfilled after dual-build copy2; must match dist/ + versioned manifest.
...
def _load_zip_helpers():
    ...
    spec = importlib.util.spec_from_file_location("installed_zip_plugin_r10", path)
    ...
    *,
    ...
    if STRICT and not ARTIFACTS_LANDED:
        raise FileNotFoundError(
            "R10_044_ARTIFACTS_PENDING_BUILD: "
...
def run_isolated_continuity_e2e(plugin_zip: Path) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r10-044-zip-e2e-") as tmp:
        tmp_path = Path(tmp)
        old_cwd = Path.cwd()
        os.chdir(tmp_path)
        try:
            ...
            try:
                ...
                # 4) Protocol-field unresolved → Provider=0 (0.4.3 retained).
                ...
                # 4b) Protocol-field registered secret → Provider=0 (0.4.4 narrow).
...
def prove_source_fallback_mutation_red(plugin_zip: Path) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r10-044-src-mut-") as tmp:
        tmp_path = Path(tmp)
        extract_root = tmp_path / "zip-extract"
...
def evaluate_r10_044_final_zip_gates(summary: dict) -> int:
...
def main() -> int:
    ...
    code = evaluate_r10_044_final_zip_gates(summary)
    if code != 0:
        print(f"R10_044_FINAL_ZIP_GATE_FAIL code={code}", file=sys.stderr)
        return code
    if not summary.get("source_fallback_mutation_red"):
        print("R10_044_SOURCE_FALLBACK_MUTATION_NOT_RED", file=sys.stderr)
        return 13
    return 0
