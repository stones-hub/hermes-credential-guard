#!/usr/bin/env python3
"""R8 0.4.2 final-ZIP E2E opt-in pytest runner.

Runs exactly ``tests/r8_042_final_zip_e2e.py`` (outside the no-build
...
E2E_MODULES: Tuple[str, ...] = ("tests/r8_042_final_zip_e2e.py",)

SAFE_DISPLAY_FLAGS = ("-q", "-v", "-x", "--collect-only")
SAFE_TB_VALUES = ("short", "long", "line", "native", "no", "auto")


class R8FinalZipRunnerArgError(ValueError):
    ...
    for arg in extra:
        ...
        if arg.startswith("--tb="):
            if arg.split("=", 1)[1] not in SAFE_TB_VALUES:
                raise R8FinalZipRunnerArgError(f"unsupported --tb value: {arg}")
            out.append(arg)
            continue
        if arg.startswith("--maxfail="):
            if not arg.split("=", 1)[1].isdigit():
                raise R8FinalZipRunnerArgError(f"unsupported --maxfail value: {arg}")
            out.append(arg)
            continue
        raise R8FinalZipRunnerArgError(
    ...
    try:
        for rel in E2E_MODULES:
            if not (REPO / rel).is_file():
                raise R8FinalZipRunnerArgError(f"E2E module missing: {rel}")
        py_argv = build_pytest_argv(extra)
    except (R8FinalZipRunnerArgError, OSError) as exc:
        print(f"R8_042_FINAL_ZIP_RUNNER_REJECT {exc}", file=sys.stderr)
        return 2
    env = build_env()
    print(
        "R8_042_FINAL_ZIP_RUNNER opt-in E2E: "
